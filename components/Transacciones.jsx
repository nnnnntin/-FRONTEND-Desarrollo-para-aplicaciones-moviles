...
const transaccionSchema = Yup.object({
  ...
  precio: Yup.string()
    .matches(/^\$\d+(\.\d{2})?$/, 'Formato de precio inválido ($XX.XX)')
    .required('Precio es requerido'),
...
const Transacciones = ({ navigation, route }) => {
  ...
  const handleCompartir = async () => {
    try {
      ...
      const fechaLimpia = transaccionValidada?.fecha?.replace(/\//g, '-') || 'documento';
  ...
  const handleVerReserva = () => {
    try {
      if (transaccionValidada?.reserva?.id) {
      ...
      Alert.alert('Error', 'Error al navegar a detalles de reserva: ' + error.message);
    }
  };

  const handleVerPago = () => {
    try {
      const pagoId = transaccionValidada?.pago?.id || transaccionValidada?.pago?._id;
      if (pagoId) {
        navigation.navigate('DetallePago', {
          pagoId: pagoId
        });
      } else {
        Alert.alert('Error', 'ID de pago no disponible');
      }
    } catch (error) {
      Alert.alert('Error', 'Error al navegar a detalles de pago: ' + error.message);
    }
  };

  const handleVerFactura = () => {
    try {
      if (transaccionValidada?.factura?.id) {
        navigation.navigate('DetalleFactura', {
          facturaId: transaccionValidada.factura.id
        });
      } else {
        Alert.alert('Error', 'ID de factura no disponible');
      }
    } catch (error) {
      Alert.alert('Error', 'Error al navegar a detalles de factura: ' + error.message);
    }
  };
  ...
  const datosTransaccion = useMemo(() => {
    ...
    return {
      fecha: transaccionValidada.fecha || new Date().toLocaleDateString('es-ES'),
      precio: transaccionValidada.precio || '$0.00',
      nombreUsuario: obtenerNombreUsuario(),
      emailUsuario: obtenerEmailUsuario(),
      metodoPago: transaccionValidada?.pago?.metodoPago?.tipo || 'Tarjeta',
      ultimosDigitos: transaccionValidada?.metodo?.ultimosDigitos ||
        transaccionValidada?.pago?.metodoPago?.detalles?.ultimosDigitos || '****'
    };
  }, [transaccionValidada, obtenerNombreUsuario, obtenerEmailUsuario]);
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        {erroresValidacion.transaccion && (
          <View style={styles.warningContainer}>
            <Ionicons name="warning" size={20} color="#f39c12" />
            <Text style={styles.warningText}>
              Algunos datos de la transacción pueden no ser válidos
            </Text>
          </View>
        )}
        ...
        <View style={styles.transaccionContainer}>
          ...
          <View style={styles.transaccionItem}>
            ...
            <View style={styles.transaccionInfo}>
              ...
              <Text style={styles.transaccionValor}>{datosTransaccion?.precio || '$0.00'}</Text>
            </View>
          </View>
          ...
          <View style={styles.transaccionItem}>
            ...
            <View style={styles.transaccionInfo}>
              ...
              <Text style={styles.transaccionValor}>{datosTransaccion?.nombreUsuario || 'Usuario'}</Text>
            </View>
          </View>

          {datosTransaccion?.emailUsuario && (
            <View style={styles.transaccionItem}>
              <View style={styles.transaccionIcono}>
              ...
              <View style={styles.transaccionInfo}>
                ...
                <Text style={styles.transaccionValor}>{datosTransaccion.emailUsuario}</Text>
              </View>
            </View>
          )}
          ...
          <View style={[styles.transaccionItem, styles.lastItem]}>
            ...
            <View style={styles.transaccionInfo}>
              ...
              <Text style={styles.transaccionValor}>
                {datosTransaccion?.metodoPago} •••• {datosTransaccion?.ultimosDigitos}
              </Text>
            </View>
          </View>
        </View>
        ...
        {transaccionValidada?.pago && (
          <View style={styles.pagoContainer}>
            ...
            <View style={styles.pagoHighlight}>
              ...
              <TouchableOpacity style={styles.botonVerDetalle} onPress={handleVerPago}>
                <Ionicons name="eye-outline" size={16} color="#2c3e50" />
        ...
        {transaccionValidada?.factura && (
          <View style={styles.facturaContainer}>
            ...
            <View style={styles.facturaHighlight}>
              ...
              <TouchableOpacity style={styles.botonVerDetalle} onPress={handleVerFactura}>
                <Ionicons name="eye-outline" size={16} color="#2c3e50" />
                <Text style={styles.textoBotonVerDetalle}>Ver detalles de factura</Text>
              </TouchableOpacity>
            </View>
          </View>
        )}
        ...
        {transaccionValidada?.reserva && (
          <View style={styles.reservaContainer}>
            ...
            <View style={styles.reservaHighlight}>
              ...
              <TouchableOpacity style={styles.botonVerDetalle} onPress={handleVerReserva}>
                <Ionicons name="eye-outline" size={16} color="#2c3e50" />
                <Text style={styles.textoBotonVerDetalle}>Ver detalles de reserva</Text>
              </TouchableOpacity>
            </View>

            {transaccionValidada.reserva.serviciosAdicionales &&
              transaccionValidada.reserva.serviciosAdicionales.length > 0 && (
                <View style={styles.serviciosContainer}>
                  <Text style={styles.serviciosTitle}>Servicios adicionales</Text>
                  {transaccionValidada.reserva.serviciosAdicionales.map((servicio, index) => {
                    try {
                      servicioAdicionalSchema.validateSync(servicio);
                      return (
